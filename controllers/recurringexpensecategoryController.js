...
const RecurringExpenseCategory = require('../models/recurringexpensecategory');

const recurringexpensecategoryController = {
   ...
   /* getJobs: async (req, res) => {
    ...
    getRecurringExpenseCategories: async (req, res) => {
        try {
           // const  {userId}  = req.userId;
            const recurringexpensecategories = await RecurringExpenseCategory.find().select('-__v');

            res.status(200).json(recurringexpensecategories);
        } catch (error) {
            res.status(500).json({ message: error.message });
        }
    }
...
module.exports = recurringexpensecategoryController;