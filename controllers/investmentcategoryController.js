...
const investmentcategoryController = {
   ...
   /* getJobs: async (req, res) => {
    ...
    getInvestmentCategories: async (req, res) => {
        try {
           // const  {userId}  = req.userId;
            const investmentcategories = await InvestmentCategory.find().select('-__v');

            res.status(200).json(investmentcategories);
        } catch (error) {
            res.status(500).json({ message: error.message });
        }
    }
